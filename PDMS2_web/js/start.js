...
(function () {
  ...
  // 輸入時鑰匙搖晃
  const uidIcon = document.querySelector(".uid-icon");
  if (uidIcon) {
    uidInput.addEventListener("input", () => {
      if (!uidIcon.classList.contains("shaking")) {
        uidIcon.classList.add("shaking");
        uidIcon.addEventListener("animationend", () => {
          uidIcon.classList.remove("shaking");
        }, { once: true });
      }
    });
  }
