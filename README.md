...
如今影像辨識的技術已非常成熟，而藉由此技術配合模型辨識，讓系統在旁人不需要具備專業背景（不一定要是職能治療師）的情況下，也能協助完成辨識與評分，結合妙妙屋環境，並具備下列優勢：
...
MySQL 紀錄單次與縱向表現，便於後續轉介與追蹤（目前資料庫與後端服務部署於實驗室 Mac mini，並透過 Tailscale 進行外網連線）
...
- **資料庫／後端主機：** MySQL 8.0+（部署主機：實驗室 Mac mini，透過 Tailscale 提供外網連線）
