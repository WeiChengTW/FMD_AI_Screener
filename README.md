...
[![CV](https://img.shields.io/badge/Computer%20Vision-OpenCV-green.svg)](https://opencv.org/)
[![Model](https://img.shields.io/badge/Model-YOLO-red.svg)](https://github.com/ultralytics)
[![Segmentation](https://img.shields.io/badge/Segmentation-SAM-purple.svg)](https://github.com/facebookresearch/segment-anything)
[![DL](https://img.shields.io/badge/Deep%20Learning-TensorFlow-orange.svg)](https://www.tensorflow.org/)
[![DL](https://img.shields.io/badge/Deep%20Learning-PyTorch-EE4C2C.svg)](https://pytorch.org/)
[![Tunneling](https://img.shields.io/badge/Networking-Tailscale-242424.svg)](https://tailscale.com/)
...
- 2026 臺灣數創大賞「智慧＋組」創業計畫書（已於 2026/7/11 提交，結果尚未公布）
- 2026 全國大專校院智慧創新暨跨域整合創作競賽（系統需求書 SRS v1.0 已於 2026/7/11 完稿，參賽中）
...
除了解決職治師分配不均的問題，不是每一個衛生所，都有適合的環境給小朋友使用。本系統的環境最初是帳篷式的妙妙屋，目前已與學校工業設計系合作，改版為拱形可組裝隔板結構：內建光源燈條確保影像辨識品質、俯視與側面雙鏡頭攝影機負責影像擷取、觸控螢幕播放故事情境並提供操作介面，並搭配統一施測環境的測驗墊與測驗輔具，提供給小朋友一個歡樂又舒適的環境，操作遊戲項目。架設和收拾只需不到五分鐘的時間就可以完成，只需要一個提袋即可攜帶全部的東西，改善了偏鄉地區復健教具搬移困難的問題。

### 3. 雲端留痕，持續追蹤

MySQL 紀錄單次與縱向表現，便於後續轉介與追蹤（目前資料庫與後端服務部署於實驗室 Mac mini M4，並透過 Tailscale 進行外網連線）
早期療育的過程，往往需要多次的回診與復健，有個將每位小朋友的歷史活動記錄下來的系統，會讓整個治療過程變得非常順利。因此結合 MySQL 資料庫連動後端系統，可以將每位小朋友每次的活動各項評分和總分儲存，並於完成該次活動之後回傳到資料庫，大大增加整個評估活動的便利性。圖片與錄影改為以雲端連結方式存取，連結格式會由後端產生簽名網址，再經由 Tailscale 可連線位址對外提供，方便在不同裝置間直接開啟與查閱。
...
系統不是只用單一模型做黑箱判斷，而是依照不同關卡把影像任務拆成多個步驟：先由 YOLO 找出積木、紙張、物件或豆子等目標，再視題型使用 SAM 精準切出區域，之後交由 TensorFlow 分類模型或幾何規則做進一步判斷，最後把結果轉成分數與標註影像。Ch5 則改為側重儀器搭配 Arduino 的即時量測流程，持續計算數量與時間，並把遊戲狀態回傳到前端做即時回饋與最終評分。這樣的設計可以貼近 PDMS-2 的評分邏輯，同時降低人工逐張判讀的負擔，也讓後續擴充新題型時可以沿用同一套分析架構。

### 6. 模組化擴充

先支援 3–6 歲 17 項目，後續擴大年齡與題組
此系統目前完成 17 個 PDMS-2 精細動作項目（Ch1 疊積木 4 項、Ch2 圖形描繪 6 項、Ch3 剪紙 4 項、Ch4 摺紙 2 項、Ch5 撿豆子 1 項），適用年齡範圍為 3-6 歲，還可以再擴增適用範圍與測試項目數量；除了支援衛生所做巡迴使用，也可以引進到幼兒園，供幼稚園老師做使用，以便提早發現病狀，盡早接受治療。
...
本系統的做法不是傳統那種冷冰冰的測驗，而是設計一個故事情境化的測驗空間「妙妙屋」。妙妙屋最初的構想是帳篷式的可攜結構，目前已與學校工業設計系合作，改版為拱形可組裝隔板結構（圖六）：內建光源燈條確保影像辨識品質，俯視與側面雙鏡頭攝影機負責影像擷取，觸控螢幕播放故事情境並提供操作介面，並搭配統一施測環境的測驗墊。小朋友一旦走進妙妙屋，就好像進入了一個故事冒險的世界，不再是單純地被要求做測驗，而是透過故事角色與關卡的引導來完成任務。前端介面包裝成「歡迎來到童話王國」的遊戲化闖關系統，小朋友登入後就像進入童話王國探險，跟著任務引導一關一關完成測驗。

<div align="center">

![圖六、設備與器材圖](picture/圖片_設備器材.png)
...
這些道具，會高度還原 PDMS-2 量表所提供教具，比如圖形的大小、寬度、積木的尺寸大小等。測驗環境外框與積木教具皆與學校工業設計系協作設計製作。測驗的平台會使用自架互動網站，嵌入到觸控螢幕讓小朋友作互動使用。
...
把結果用簡單明瞭的方式呈現給老師或家長，可以依照著評分標準來決定小朋友是否需要近一步的評估或是治療，產出家長版與專業版報告範本（短版指標、長版歷程）。使用者依權限分為四級：兒童（受測者）、家長（管理者等級 1，僅能查看自家孩童結果）、施測人員（管理者等級 2，執行測驗與查看評分）、管理者／醫護（管理者等級 3，管理帳號、查詢紀錄與匯出報表）。
...
- **Ch4 折紙題：** 以彩色邊緣與最大面積四邊形偵測為主，搭配幾何量測與影像分析判定折線與紙張形狀。
- **Ch5 側重儀器題：** 以側重儀器搭配 Arduino 進行即時量測，持續計算數量與時間，並結合遊戲狀態紀錄做即時回饋與最終評分。

這些流程共同的設計原則是「先偵測、再分割、後分類、最後用規則或量測轉成分數」，使模型結果更接近臨床或教學情境下的人工判讀方式。
...
系統已產出可運作之完整原型（硬體場域＋AI 模型＋雲端資料庫＋前後台介面），並與合作醫療院所進行 AI 判讀結果與職能治療師人工評估的交叉驗證，目前的模擬測試數據包括：

- 兩點連線任務：以形態學影像運算達成 89% 偵測準確率
- 手繪圓形任務：微調後的 ResNet-50 分類模型達成 95.8% 辨識準確率
- 系統規格要求各測驗項目之 AI 辨識與評分準確率須達 85% 以上

## AI 生成建議系統（RAG）

系統將大語言模型與 RAG（檢索增強生成）架構導入早療報告系統。當 MySQL 資料庫偵測到孩童某項評分低於 2 分（弱項）時，AI 建議模組會自動啟動：

- **知識庫：** 以 PDMS-2 官方手冊與相關醫學研究論文建立 ChromaDB 向量資料庫，Embedding Model 採用 all-MiniLM-L6-v2。
...
- **呈現方式：** 職能治療師可遠端查看孩童的五大能力分布雷達圖與詳細歷史紀錄；家長則在網頁上收到易懂的個人化建議報告，包含各關卡得分、測驗日期與原始／結果圖。

<div align="center">

![圖十、個人化 RAG 評估報告架構圖](picture/圖片_RAG架構.png)
...
- **後端主機：** Flask 與 MySQL 部署在實驗室 Mac mini M4，外部連線透過 Tailscale 轉成可訪問的位址。
- **媒體存取：** 圖片與影片不再以本機路徑直接讀取，而是由後端產生簽名連結，前端可直接透過對外網址開啟對應檔案。
- **Ch5 量測流程：** 側重儀器搭配 Arduino 即時回傳數量與時間，系統再根據遊戲狀態與門檻值做即時回饋與最終評分。
...
2026/5/9 系統已於社區關懷據點進行真實場域測試（教育部大學社會責任實踐計畫「在地扎根．社會共生－優質教育、代間共融、AI 智慧療育計畫」，長庚科技大學主辦），實際讓小朋友操作測驗設備，並留下落地驗證紀錄。

<div align="center">

![圖十一、落地測試（設備架設）](picture/圖片_落地測試1.jpg)
...
![圖十二、落地測試（小朋友實際操作）](picture/圖片_落地測試2.jpg)
...
- **資料庫／後端主機：** MySQL 8.0+（部署主機：實驗室 Mac mini M4，透過 Tailscale 提供外網連線）
- **圖片／影片存取：** 雲端連結式存取，配合 Tailscale 轉換後的可連線位址
...
- 兒童個資與測驗紀錄依權限分級存取，並具每日自動備份與異地副本

## 開發工具及相關說明

### 核心技術框架

- **機器學習／深度學習：** YOLO 系列物件偵測與分割、SAM、TensorFlow、PyTorch
